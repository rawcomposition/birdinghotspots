...
import { getRegionRoadside } from "lib/sqlite";
...
export default function RoadsideBirding({ region, hotspots }: Props) {
  return (
    <div className="container pb-16 mt-12">
      <Title>{`Roadside Birding - ${region.detailedName}`}</Title>
      <PageHeading region={region}>Roadside Birding</PageHeading>
      <div className="md:flex gap-8 items-start mb-8">
        ...
        <div>
          ...
          <p className="mb-4">
            ...
            <Link href={`/region/${region.code}/accessible-facilities`}>handicap accessible facilities</Link>.
          </p>
        </div>
      </div>
...
export const getServerSideProps: GetServerSideProps = async ({ query }) => {
  ...
  const hotspots = (getRegionRoadside(regionCode) || []) as HotspotsByCounty;
