...
export default function ContentConflict({ locationId, onSave }: Props) {
  ...
  if (!data || !data.hotspot) return <p className="text-gray-500">No general hotspot found.</p>;
    ...
    <div>
      <div className="grid grid-cols-2 gap-4 mb-4">
        <h4 className="font-bold text-sm text-gray-700">
          ...
          <Link href={`/group/${locationId}`} target="_blank" className="ml-2 font-normal text-sm text-[#4a84b2]">
            View
          </Link>
        </h4>
        <h4 className="font-bold text-sm text-gray-700">
          ...
          {data.primaryHotspotUrl && (
            <>
              <span className="ml-3 font-normal text-gray-400">–</span>
              <Link href={data.primaryHotspotUrl} target="_blank" className="ml-2 font-normal text-sm text-[#4a84b2]">
                View
              </Link>
            </>
          )}
      ...
      {fieldLabels.map(({ key, label }) => {
        ...
        const identical =
          !!groupVal && !!hotspotVal && groupVal === hotspotVal && !(key === "restrooms" && groupVal === "Unknown");
        return (
          <div key={key} className="mb-4">
            <div className="mb-1 flex items-center gap-2">
              <span
                className={`text-xs font-semibold px-1.5 py-0.5 rounded ${
                  conflict ? "text-red-600 bg-red-50" : "text-gray-500 bg-gray-100"
                }`}
              >
                {label}
              </span>
            ...
            <div className="grid grid-cols-2 gap-4">
              {key === "restrooms" ? (
                <>
                  <RestroomRadio
                    value={groupEdits?.restrooms || "Unknown"}
                    onChange={(v) => handleGroupChange("restrooms", v)}
                    name="group-restrooms"
                  />
                  <RestroomRadio
                    value={hotspotEdits?.restrooms || "Unknown"}
                    onChange={(v) => handleHotspotChange("restrooms", v)}
                    name="hotspot-restrooms"
                  />
                </>
              ) : (
                <>
